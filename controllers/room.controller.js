...
const { uploadImages } = require("../utils/imgbb");
...
const createRoom = async (req, res) => {
  try {
    ...
    if ((req.files?.length || 0) > 8) {
      return response.error(res, "Eng ko'pi 8 ta rasm yuklash mumkin");
    }
...
const updateRoom = async (req, res) => {
  try {
    ...
    const retainedImages = Array.isArray(updates.images)
      ? updates.images
      : current.images;
    if (retainedImages.length + (req.files?.length || 0) > 8) {
      return response.error(res, "Eng ko'pi 8 ta rasm saqlash mumkin");
    }
